# MediaTek MT7902 (Filogic 310) Bluetooth Driver

## Overview

This patches the Linux `btusb`/`btmtk` kernel modules to add Bluetooth support for the **MediaTek MT7902** (Filogic 310) combo WiFi/Bluetooth chip. The MT7902 BT interface is exposed as a USB device (`13d3:3579`, IMC Networks Wireless_Device).

As of February 2026, MT7902 Bluetooth support has not yet been merged into mainline Linux. The patches here add it to kernel 6.17.7.
...
The existing `btusb` module does match the device via a generic vendor wildcard (`13d3:*`), but two things are missing:
